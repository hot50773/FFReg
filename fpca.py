...
class Fpca(object):
    ...
    def __Fit_Cov(self, x, y, x0, candidate_h, binning, ker_fun, bw_select):
        ...
        fit_yy = lpr.Lpr_For_Bin([bin_yy.reshape(np.tile(self.__grid_shape, 2)), bin_xx.reshape(np.tile(self.__grid_shape, 2))],
                                 np.tile(self.__bin_width, 2),
                                 h = self.cov_bw,
                                 ker_fun = ker_fun).reshape(np.tile(np.prod(self.__grid_shape), 2))
...
#        self.cov_fun = np.matmul(np.matmul(self.eig_fun.T, np.diag(self.eig_val)), self.eig_fun)
    ...
    def __Fit_Sigma2(self, x, y, x0, candidate_h, binning, bin_weight, ker_fun, bw_select):
        ...
        sigma2_t = (self.cov_dia - np.diag(restruct_cov_fun)).reshape(self.__grid_shape)
        for i in range(self.__d):
            left, right = interval.take(i, 1)
            sigma2_t = sigma2_t.take(np.arange(left, right), i)
        self.sigma2 = (sigma2_t).mean()
        self.sigma2 = 1e-6 if self.sigma2 < 0 else self.sigma2

    def __Fit_Fpc_Scores(self, x, y):
        n = y.shape[0]
        non_nan_value = ~np.isnan(y)
        mean_inter_fun = sp.interpolate.RegularGridInterpolator(self.__grid, self.mean_fun.reshape(self.__grid_shape), bounds_error = False)
        ...
        for i in range(n):
            ...
            fit_cov_inv = np.linalg.pinv(np.matmul(phi_i.T * self.eig_val, phi_i) + np.identity(y_i.size) * self.sigma2)
    ...
    def Restruect_Fun(self, x, y):
        ...
        return([fpc_scores, self.mean_fun + np.matmul(fpc_scores,
                            self.eig_fun.reshape(self.num_eig_pairs, -1)).reshape(np.append(-1,
                                                                                    self.__grid_shape))])
