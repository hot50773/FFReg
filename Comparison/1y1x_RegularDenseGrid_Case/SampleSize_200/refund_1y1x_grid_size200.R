...
{ 
  ...
  grid_width <- t_in[2] - t_in[1]
  MISE <- mean(apply((fit_test_Y - real_test_Y)^2 * grid_width, 1, sum))
  if(plot_result){
    for(j in 1:num_plot){
      windows()
...
for(i in 1:sim_rep_time){
  X_obs_tran[[i]] <- read.csv(paste('d://Work_Jupyter//敏//SIM_1y1x_grid1_size200//tran X of samp',i), 
                        quote = '\n', sep = ' ', header = F)
  Y_obs_tran[[i]] <- read.csv(paste('d://Work_Jupyter//敏//SIM_1y1x_grid1_size200//tran Y of samp',i), 
                        quote = '\n', sep = ' ', header = F)
  X_obs_test[[i]] <- read.csv(paste('d://Work_Jupyter//敏//SIM_1y1x_grid1_size200//test X of samp',i),                           quote = '\n', sep = ' ', header = F)
  Y_real_grid_val[[i]] <- read.csv(paste('d://Work_Jupyter//敏//SIM_1y1x_grid1_size200//real_on_grid Y of samp',i), 
                             quote = '\n', sep = ' ', header = F)
...
#write(refundFPC_MISE, 'd://Work_Jupyter//敏//SIM_1y1x_grid1_size200//refundFPC_MISE_200size', sep = ' ')
#write(refundFF_MISE, 'd://Work_Jupyter//敏//SIM_1y1x_grid1_size200//refundFF_MISE_200size', sep = ' ')
